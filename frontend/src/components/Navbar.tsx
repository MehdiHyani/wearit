      ...
      <div className="px-6 py-3 md:flex md:justify-between md:align-items">
        ...
        <form className="centered" onSubmit={handleSubmit}>
          ...
          <select className="px-[12px] py-[6px] h-[38px] w-auto text-white border border-solid border-primary bg-black rounded-l-lg cursor-pointer">
            {data && data.map((e, i)=> {return <option key={i} value={e.STR_ID}>{e.STR_NAME}</option>})}
          </select>
          
          <input onChange={(e) => setSearch(e.target.value)} className="form-control relative flex-auto min-w-0 w-[600px] block px-3 py-1.5 text-base font-normal text-gray-700 bg-white bg-clip-padding border border-solid border-black transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none" placeholder="Search for a product... "/>
