...
const Navbar = () => {
    ...
    const [search, setSearch] = useState(searchUrlQuery.slice(1) ?? '');
                ...
                <nav className="bg-tertiary">
                    <div className="px-6 py-3 md:flex md:justify-between md:align-items">
                        ...
                        <form className="centered" onSubmit={handleSubmit}>
                            ...
                            <input value={search} onChange={(e) => setSearch(e.target.value)} className="form-control relative flex-auto min-w-0 w-[600px] block px-3 py-1.5 text-base font-normal text-gray-700 bg-white bg-clip-padding border border-solid border-black transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none" placeholder="Search for a product... " />
                            <button onClick={() => navigate({ pathname: '/products', search })} type="submit" className="btn px-6 py-2.5 bg-secondary text-black font-medium text-xs leading-tight uppercase rounded-r-lg shadow-md hover:bg-primary hover:shadow-lg focus:bg-orange-600 hover:text-white focus:shadow-lg focus:outline-none focus:ring-0 active:bg-blue-800 active:shadow-lg transition duration-150 ease-in-out flex items-center border border-solid border-black">
